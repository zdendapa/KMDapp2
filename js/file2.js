...
function doReadFile(e) {
    fileSystem.root.getFile("TMMHExport.xml", {create:false}, readFile, onError);
}
...
function workOn(dir,type) {
    ...
    if(type=="read")
    {
        dir.getFile("TMMHExport.xml", {create:false}, readFile, onError);
    }
             ...
             */
                ...
                for (var i=0; i<len; i++){
                    if(results.rows.item(i).shid!=shid)
                    {
                        ...
                        xmlString += "<sheet><header><shid>"+results.rows.item(i).shid+"</shid><category>" + results.rows.item(i).category + "</category><planSpend>"+results.rows.item(i).planSpend+"</planSpend><code>"+results.rows.item(i).code+"</code></header><tableData>";
                        shidBefore = true;
                        shid = results.rows.item(i).shid;
                    }
                    xmlString += "<row>";
                    xmlString += "<rowID>"+results.rows.item(i).rowid+"</rowID><date>"+results.rows.item(i).dater+"</date><paid>"+results.rows.item(i).paid+"</paid><desc>"+results.rows.item(i).desc+"</desc><ref>"+results.rows.item(i).checkRef+"</ref>"
